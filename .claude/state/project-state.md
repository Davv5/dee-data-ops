...
- **`fct_calls_booked.{assigned_user_sk,pipeline_stage_sk}` are wired in CODE** (PR #123) — and the cascade that kept them 100% NULL in prod is now in resolution path. PR #123's selection rule (most-recent opp where `opportunity_created_at <= booked_at`, `opportunity_id desc` tiebreaker) compiles cleanly; the 100% NULL state surfaced by PR #135's verification baseline traced to a missing `users` entry in `bq-ingest` service `GHL_OBJECT_TYPES` env (fixed via `00099-jez`). Diagnostic attribution only — Speed-to-Lead numerator still sources first-touch identity from `raw_ghl.conversations`/`fct_outreach`. Verification (next prod nightly): `assigned_user_sk` distribution should flip from 100% NULL to non-trivial non-NULL once `dim_users` populates from the now-flowing `raw_ghl_v2.users` view.
...
- **2026-04-30 mid-day (latest)** — PR #135 mart-collapse PR-1 in flight (`booking_time_opportunity_id` added to `fct_calls_booked`; 4 commits incl. adversarial+7-persona review fixes + David-approved rename + verification artifact). Verification baseline surfaced 100% NULL `assigned_user_sk` in prod despite PR #123 wiring; investigation traced to missing `users` in `bq-ingest` service `GHL_OBJECT_TYPES` env. Fix shipped as `bq-ingest-00099-jez` (env-var-only, no code change). Next `:20 UTC` tick polls users; next prod dbt nightly should flip `assigned_user_sk` from 100% NULL. Empirical anchor + investigation pattern captured in `.claude/rules/bq-ingest.md` Lessons-Learned ("service-vs-job env-var divergence").
...
- Last regen: 2026-04-30 mid-day UTC (post-PR-#135 in-flight + bq-ingest `00099-jez` deploy fixing `users` polling).
- WORKLOG: skipped — session output captured by PR #135 (description + 4 commits including 7-persona ce-code-review fixes) + bq-ingest service deploy (revision history + this regen) + `.claude/rules/bq-ingest.md` Lessons-Learned entry capturing the empirical anchor. Per `.claude/rules/worklog.md` routing table: PR descriptions + service deploy log + rule file + this regen replace a WORKLOG entry.
