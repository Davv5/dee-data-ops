...
_Last regenerated: 2026-04-28 (Phase B build is live: PR #84 shipped `stg_fanbasis__transactions` + the real `fct_revenue` Stripe+Fanbasis union)_

## Where we are

- **Phase B (Layer Build) is live.** First ticket shipped: PR #84 (merged `5776a7b`) — `stg_fanbasis__transactions` view + `fct_revenue.sql` placeholder CTE replaced with a real `ref()`. Fanbasis arm is the live forward-going revenue contributor (466 historical rows / $170,712 gross / 9 refunds, Apr 2025 → Apr 2026); Stripe arm preserved untouched for historical rows.
- **Next Phase B ticket (queued):** extend `bridge_identity_contact_payment` with a Fanbasis-aware match-method (UNION-ALL Fanbasis arm matched on `fan.email` / `fan.phone` against existing tier hierarchy). Without it, Fanbasis rows in `fct_revenue` surface as `bridge_status='unmatched'`. Independent of GHL trusted-copy decision.
...
- **Phase B.1 artifacts (on main as of `5776a7b`):** `2-dbt/models/staging/fanbasis/stg_fanbasis__transactions.sql` + `_fanbasis__models.yml`; `2-dbt/models/warehouse/facts/fct_revenue.sql` (Fanbasis CTE is now a real `ref()`)
