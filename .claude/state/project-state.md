...
- **Buyer-grain lead magnet mart:** `2-dbt/models/marts/lead_magnet_buyer_detail.sql` has one row per matched paid contact. Use it for buyer counts, first purchase revenue, revenue per buyer, payments per buyer, days to first purchase, booking-before-purchase, and latest known magnet before first purchase.
- **Lead magnet taxonomy:** `2-dbt/seeds/lead_magnet_pipeline_taxonomy.csv` classifies 36 current GHL pipelines by category and offer type. Category = true lead magnet, launch/event, waitlist, or sales/operating pipeline. Offer type = prompt pack, template, guide/doc, resource list, training/class, video/replay, giveaway, community, waitlist, launch event, or sales pipeline. All rows are initially `taxonomy_confidence = inferred_from_name`; review with David before calling the taxonomy final.
...
- **Buyer truth:** 869 buyers, 1,372 paid payments, $263,538.96 first-purchase net revenue, $356,785.24 total net revenue after refunds, 33.3% multi-payment buyers, 72.4% with a booking before first purchase, 79.5% with a latest prior magnet before first purchase.
