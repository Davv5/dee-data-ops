...
- **Audit `*_RUN_MODELS_AFTER_INCREMENTAL` env-vars on bq-ingest service.** Two overrides remain set to `true` on the live service (verified 2026-04-29 post-fathom-fix): `GHL_RUN_MODELS_AFTER_INCREMENTAL=true` and `CALENDLY_RUN_MODELS_AFTER_INCREMENTAL=true`. Per `.claude/rules/bq-ingest.md` §"Hourly HTTP path skips heavy model refresh" these should be `false` (the code default) — if either source's `run_models()` exceeds the 180s `GUNICORN_CMD_ARGS=--timeout 180` worker budget, the hourly scheduler fails the same way fathom did. Calendly demonstrably fits today (45s observed in the calendly-drain verification earlier this session); GHL durations are unverified. Audit: time `/refresh-ghl-models` and `/refresh-calendly-models` against the live service; for any source that exceeds the worker budget, remove the override (`gcloud run services update bq-ingest --remove-env-vars=<X>_RUN_MODELS_AFTER_INCREMENTAL`). Also: the `pipeline-marts-hourly` Cloud Run Job's `run_marts_with_dependencies` only refreshes `model.calendly` + `model.fathom` upstream — `model.ghl` does NOT run there, so removing the GHL override means GHL models refresh via `pipeline.full` (daily Cloud Run Job) only. Verify that's acceptable for the GHL refresh cadence before flipping.
