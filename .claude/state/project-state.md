...
- **`bq-ingest` runs revision `bq-ingest-00083-5kk` at 100% traffic.** Deployed 2026-04-29 evening after PR #127 (the corrected `operator: range` filter shape — supersedes the earlier `00093-xiv` deploy that shipped the wrong `gt` operator). `00093-xiv` retained at 0% as rollback. **Live verified:** 18:55 UTC redeploy + `/ingest-ghl` probe returned `entity_results.contacts.status=COMPLETED`, and 48 fresh contacts landed in `Raw.ghl_objects_raw` within minutes — first non-stale contacts ingest in 26+ hours.
...
- **2026-04-29 evening (latest)** — Closed the dbt-prod-fully-broken-since-consolidation arc with **dbt nightly green for the first time since at least 2026-04-25** (run `25130795764`: PASS=288 WARN=3 ERROR=0 / 291). Twelve PRs total this session, four of them exposing cascading post-U2 consolidation residue: PR #126 (Workload Identity Federation cutover for prod auth — sa-transform via OIDC), PR #127 (corrected GHL contacts filter to `operator: range` with inner `{gt, lt}` — earlier `gt` fix in #118 was wrong, live deploy revealed it), PR #128 (repointed dbt sources at `raw_ghl_v2` views over `Raw.ghl_objects_raw` — abandoned `raw_ghl.*` tables stopped writing 2026-04-19), PR #129 (Y1 schema cutover: `staging`/`warehouse`/`marts` lowercase → `STG`/`Core`/`Marts` capitalized to match consolidated project's actual datasets). Architectural follow-ups deferred: Y3 (dbt owns Marts entirely, retire bq-ingest's inline marts.sql) is the cleanest endpoint but multi-day; orphan lowercase `staging`/`warehouse`/`snapshots` datasets need explicit decommission decision.
...
- **bq-ingest production:** Cloud Run service `bq-ingest`, revision `bq-ingest-00083-5kk` (deployed 2026-04-29 evening, includes PRs #118 + #119 + #127). URL: `https://bq-ingest-mjxxki4snq-uc.a.run.app`. `bq-ingest-00093-xiv` (the prior deploy with the broken `gt` operator) retained at 0% as rollback.
