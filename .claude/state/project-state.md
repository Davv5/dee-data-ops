...
- **`fct_calls_booked.{assigned_user_sk,pipeline_stage_sk}` are wired in CODE** (PR #123) — but **100% NULL in prod** as of 2026-04-30 baseline (5,487 / 5,487 rows). PR #123's selection rule (most-recent opp where `opportunity_created_at <= booked_at`, `opportunity_id desc` tiebreaker) compiles cleanly; the 100% NULL state means an upstream join axis is broken — likely either `stg_ghl__opportunities` is empty post-Y1, or `opp.contact_id` doesn't match `dim_contacts.contact_id`, or `opp.assigned_user_id` doesn't match `dim_users.user_id`. Diagnostic queries enumerated in PR #135's plan doc §11. **Investigation needed before PR-2 of mart-collapse can demonstrate value.**
