...
_Last regenerated: 2026-04-28 (Phase B.4 ships: `revenue_detail` net-of-refunds extension)_
...
- **Local dev env stale** — `.env` still points `GCP_PROJECT_ID_DEV=dee-data-ops`; consolidated project is `project-41542e21-470f-4589-96d`. Documented workaround: ADC + oauth profile at `/tmp/dbt-ci-local/profiles.yml` (see `feedback_local_ci_bypass.md` memory). Permanent fix is the `.env` update + cross-project SA key.
...
- Last regen: 2026-04-28 (post Phase B.4 ship)
- WORKLOG: skipped — Phase B.4 narrative captured in PR #92 description (two-commit sequence: initial extension + defensive-CASE refactor) + a PR comment with local-CI evidence (`PASS=288 WARN=3 ERROR=0`) when GitHub Actions hit `degraded_performance` and queued the second run. The local-CI-bypass operational pattern is captured as memory `feedback_local_ci_bypass.md` (cross-session learning, not append-only narration). No residual content needing WORKLOG entry.
