...
- **`fct_calls_booked` SK gaps** — `assigned_user_sk` + `pipeline_stage_sk` still hardcoded `cast(null as string)` at `2-dbt/models/warehouse/facts/fct_calls_booked.sql:70-71`. CTEs defined but unused. Per CLAUDE.local.md the join axis (`stg_calendly__event_invitees.invitee_email_norm`) does now exist (`contact_sk` resolves through it) — so this is now actionable; matters for SDR/AE attribution slices in dabi.
