...
- **`*_RUN_MODELS_AFTER_INCREMENTAL` audit — closed 2026-04-29.** Both `GHL_RUN_MODELS_AFTER_INCREMENTAL` and `CALENDLY_RUN_MODELS_AFTER_INCREMENTAL` removed from the bq-ingest service (now use the in-code default of `false`). PR #115 added `model.ghl` to `run_marts_with_dependencies` so GHL refresh continues hourly via the marts Cloud Run Job context (preserves `mrt_speed_to_lead_daily` cadence). Verified post-flip: `/ingest-calendly` returns 200 in 26s (`models_refreshed=false`); `/ingest-ghl` returns 200 in 42s (`models_refreshed=false`); `pipeline-marts-hourly` Job manually triggered post-deploy and succeeded with the new Calendly→Fathom→GHL→Marts sequence. Architectural rule in `.claude/rules/bq-ingest.md` §"Hourly HTTP path skips heavy model refresh" now fully applied across all three live sources (Fathom, Calendly, GHL).
- **GHL contacts upstream 422 (NEW 2026-04-29).** Surfaced during the GHL post-flip verification curl. `/ingest-ghl` reports `entity_type=contacts` failed with `GoHighLevel request failed (status=422, "Invalid Operator (gte) passed for field date_updated")`. This is a GHL **upstream API behavior change** — not caused by removing the env-var override. Other GHL entities (opportunities, forms, form_submissions, pipelines, pipeline_stages, outbound_call_logs, conversations) continue to ingest cleanly. Contacts ingestion has presumably been failing for some time; the model-refresh-skip just made the per-entity error more visible in the response. Investigation: which `gte` operator does `ghl_pipeline.py` pass on `date_updated`, and what does GHL's current API expect (LeadConnectorHQ may have moved away from `gte` toward range-based filters).
