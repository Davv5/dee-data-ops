...
Load when working on any file under `1-raw-landing/` (the `(id, _ingested_at, payload)` GH-Actions extractor path) or `services/bq-ingest/` (the Cloud Run Flask + Cloud Run Jobs path consolidated from `gtm-lead-warehouse` per `docs/plans/2026-04-28-bq-ingest-consolidation-plan.md`). The bare `ingest/` and `sources/` paths in the frontmatter glob are portability aliases — accepted for sibling projects, not active directories in this repo.
