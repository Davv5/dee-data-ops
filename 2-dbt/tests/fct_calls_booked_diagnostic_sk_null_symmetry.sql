-- Singular test: when selected_opportunity_id is NULL (no opp picked at
...
-- all three columns are NULL. A row where selected_opportunity_id is NULL but
...
--
-- One-way implication only. The reverse (selected_opportunity_id NOT NULL ⟹
...
-- Those cases keep selected_opportunity_id populated while the SK is NULL via
...
where
    selected_opportunity_id is null
    and (
