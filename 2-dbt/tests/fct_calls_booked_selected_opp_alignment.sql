-- Singular test: when selected_opportunity_id is non-NULL, the user_sk
...
-- (selected_opportunity_id, assigned_user_sk, pipeline_stage_sk) are
-- derived from the same opportunity_at_booking pick.
--
-- Mechanism: re-derive what assigned_user_sk and pipeline_stage_sk SHOULD
-- be by joining stg_ghl__opportunities on selected_opportunity_id, then
-- dim_users / dim_pipeline_stages on the opp's assigned_user_id and
-- (pipeline_id, pipeline_stage_id). For non-NULL selected_opportunity_id
...
with fact as (
    ...
    where selected_opportunity_id is not null
),

re_derived as (
    select
        fact.booking_sk,
        fact.selected_opportunity_id,
    ...
    from fact
    left join {{ ref('stg_ghl__opportunities') }} as opp
        on opp.opportunity_id = fact.selected_opportunity_id
