...
def main() -> None:
    ...
    default_run_id = f"ghl-{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}-{uuid.uuid4().hex[:8]}"
    run_id = os.getenv("GHL_BACKFILL_RUN_ID", default_run_id)
    max_pages_per_object = int(os.getenv("GHL_MAX_PAGES_PER_OBJECT", "0"))
    run_models_after = os.getenv("GHL_RUN_MODELS_AFTER", "true").lower() == "true"
    object_types = parse_object_types()

    print(
        f"Starting GoHighLevel backfill run_id={run_id} "
        f"location_id={GHL_LOCATION_ID} object_types={','.join(object_types)}"
    )
    ...
    if run_models_after:
        executed = run_models()
        print(f"GoHighLevel models refreshed successfully. statements_executed={executed}")

    print(f"GoHighLevel backfill completed for run_id={run_id}")
