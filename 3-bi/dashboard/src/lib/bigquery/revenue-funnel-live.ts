...
export function buildRevenueFunnelQueries(timeRange: RevenueFunnelTimeRange) {
  const revenueTable = `
    (
      SELECT
        *,
        COALESCE(NULLIF(best_available_operator_name, ''), 'Unassigned / unknown') AS credited_closer_name,
        COALESCE(NULLIF(best_available_operator_role, ''), 'unknown') AS credited_closer_role,
        COALESCE(NULLIF(best_available_operator_source, ''), 'unassigned') AS credited_closer_source,
        CASE
          WHEN COALESCE(NULLIF(best_available_operator_source, ''), 'unassigned') = 'unassigned'
            THEN 'missing'
          WHEN best_available_operator_role = 'Closer'
            THEN 'high'
          WHEN best_available_operator_role IS NOT NULL
            AND best_available_operator_role != 'unknown'
            THEN 'medium'
          ELSE 'low'
        END AS credited_closer_confidence,
        COALESCE(NULLIF(latest_prior_assigned_user_name, ''), NULLIF(first_touch_user_name, ''), 'Unassigned / unknown') AS credited_setter_name
      FROM ${tableRef()}
    )
  `;
