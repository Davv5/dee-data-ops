# FRIDAY — HUD + Brain integration

The gold HUD (`friday/`, Electron) is the **face**. The local LLM
...
The status line in the HUD reads **FRIDAY · ONLINE** when the brain is reachable,
**FRIDAY · LISTENING** when it's local-only, and **FRIDAY · THINKING** mid-turn.
...
| `friday/src/brain.js` | main-process HTTP client to the brain |
| `friday/main.js` | `brain:ask` / `brain:health` IPC + applies task actions |
| `friday/preload.js` | exposes `askBrain` / `brainHealth` to the renderer |
| `friday/renderer/js/quickadd.js` | routes directives to the brain, falls back to NL task add |
...
# terminal 2 — the HUD
cd ~/Documents/dee-data-ops/friday && open FRIDAY.app
```
