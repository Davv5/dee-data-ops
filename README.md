...
| `3-bi/dashboard/` | BI Tools (active plan)    | New click-around dashboard product scaffold. D-DEE is the first tenant; Speed-to-Lead is the first chapter. |
| `3-bi/metabase/` | BI Tools (retired)         | Historical reference. Self-hosted Metabase OSS ran D-DEE through 2026-04. Use it for metric/card parity only, not as the current product surface. |
...
3-bi/                                     # BI Tools (pipeline stage 3)
  dashboard/                              # Next.js dashboard product scaffold
  metabase/
...
CI, docs, ingestion/runtime checks, and dashboard data access. Legacy
`dee-data-ops` and `dee-data-ops-prod` references still exist as rollback,
parity, and raw-history references.

Full provenance map: [`docs/discovery/cloud-project-provenance-map.md`](docs/discovery/cloud-project-provenance-map.md).

Never rely on the local gcloud default for D-DEE commands; pass
`--project=project-41542e21-470f-4589-96d` or
`--project_id=project-41542e21-470f-4589-96d` explicitly.
...
| Dashboard product | `project-41542e21-470f-4589-96d` | New `3-bi/dashboard/` should read current-project contracts/report tables. |
| Legacy prod / rollback reference | `dee-data-ops-prod` | Old clean-rebuild prod data plus legacy minute-level jobs. Audit before touching. |
| Legacy dev / raw reference | `dee-data-ops` | Large old raw datasets remain here. Audit before deleting or claiming parity. |
...
- **Source-ID comparison audit:** [`docs/discovery/source-id-comparison-audit-2026-05-01.md`](docs/discovery/source-id-comparison-audit-2026-05-01.md) — pause-safety evidence for legacy raw writers
- **Pause legacy Calendly poller:** [`docs/runbooks/pause-legacy-calendly-poll.md`](docs/runbooks/pause-legacy-calendly-poll.md) — reversible procedure for the first legacy pause candidate
